...
- **I-6 過剰謙譲・冗長敬語（★日本語固有）** → **v1.1 本項化（done, 2026-07-12 taxonomist 審査済み）**。「させていただく」「所存であります」「いただく必要がございます」。S2、誤検出リスク高につき冗長重ね掛け・密度(3+)のみ検出。`hits: 1run(002 で3インスタンス)` 出所 detector-002, rewriter-002。※単一ジャンル依存の留保付き昇格。次 run 以降の敬体一般での再現を継続監視。
- **ジャンル別「儀礼定型 allowlist」** → **採用（done, I-6 直下に明記）**。公的文書の「お詫び申し上げます／賜りますよう／何卒よろしく／謹んで／拝啓・敬具」等を過検出しない機構。detector は `genre: public_document` のとき allowlist を減点対象から除外。`hits: 1run(002 で4agent)` 出所 detector-002, rewriter-002, fidelity-002, naturalness-002。
