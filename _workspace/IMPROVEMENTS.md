...
### 【最優先・P1 taxonomy 正確性】A-8 が裸受動「〜される」を扱えない `status: done`（適用 run 2026-08-03, taxonomy v1.1） `hits: 1run(2agent)`
- **[適用 2026-08-03]** taxonomist 審査を経て taxonomy を **v1.1** へ改訂。A-8 を **A-8a 裸受動「〜される」の連鎖 [S1相当・密度判定型]** と **A-8b 「〜によって」by-passive [S2]** に分割。`A-8` 単独指定は両者を包含する上位ラベルとして後方互換。A-8a に脱受動4処方（能動化/可能形/連体化/自動詞化）を inline 記載。**残: playbook/detector/rewriter プロンプトの A-8a/A-8b 出し分け追随は次段（上記「A-8 脱受動の4パターン決定表」ready 項）。**
