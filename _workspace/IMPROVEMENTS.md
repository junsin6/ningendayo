...
### 新パターン候補（taxonomist 審査中 → v1.1 拡張候補欄へ）
- **A系複合「抽象主語＋可能形」** 技術解説の強シグネチャ。実例(001)「メトリクスを使えば…可視化できます／ログには…残ります／トレースをたどれば…追跡できます」の同型3連。単独 A-10/A-5 より露見度大。複合タグ or 新 A-14。`hits: 1` 出所 detector-001
- **公的文書 過剰敬語連鎖「〜におかれましては」前置き** 実例(002)2回。E-2 とは別軸（敬語過剰度）。`hits: 1` 出所 detector-002
- **公的文書 依頼定型の全段落反復「〜いただきますようお願い申し上げます」** 実例(002)4回（ご了承/ご確認/お問い合わせ/ご活用）。密度3回以上で発火。`hits: 1` 出所 detector-002
- **受動＋状態叙述複合「〜とされております」** A-8＋A-6＋敬体の三重複合（公文書固有）。実例(002)3件。`hits: 1` 出所 detector-002
- **D系 severity の反復段階化** 「〜と言えます」等を単発=S2/2回以上=S1。実例(001) 受動除去の過程で D-1 を新規混入（AI クセの付け替え）。S1/S2 解釈で受理/差し戻しが反転する境界。`hits: 1` 出所 naturalness-001
