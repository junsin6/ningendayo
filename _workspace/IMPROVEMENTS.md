...
- **K 系: 過剰敬語・二重敬語・定型末尾の過剰反復** ★日本語固有 `hits: 1run(4agent)` `status: done(v1.1 昇格・2026-08-06 run 002)` → **APPLIED**
  - taxonomist 審査承認。taxonomy を **v1.1** へ昇格し K-1/K-2 を正式カテゴリ化、playbook v1.1 に K レシピ＋公的文書ジャンル別ガード追加、全ファイルのカテゴリ数を 11/A〜K に整合。
  - taxonomist 追加提案（次回候補）: ①K-2 と I-3/I-4 の同居優先順位ルール（敬語積層あれば K-2 を親・I 系を子）をスキーマに明記 ②`meta.genre`（公的文書/コラム/ブログ/レポート）フィールド導入で K 保持数をジャンル関数化 ③高格定型辞書（拝察いたします/幸甚に存じます/ご査収のほど）の拡充。
