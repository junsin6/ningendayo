...
* 変更率を監視: `change_rate` 30% 超で警告。**中断は `change_rate` ではなく `semantic_edit_ratio`（意味改変 edit 比率）50% 超で `hold_and_report`**（IMP-001）。`change_rate` 30〜50% でも `delete_rate ≫ insert_rate`（削除主導）かつ後段で fidelity=pass・自然度 A/B なら `override accept`（理由を summary に明記）。
...
| `change_rate` 30〜50% だが削除主導 + fidelity=pass + 自然度 A/B | `override accept`（IMP-001） | `final.md` 出力。change_rate 超過の理由を summary に明記 |
