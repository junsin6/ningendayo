...
* 変更率を監視: 判定主指標は `semantic_change_rate`（意味置換のみ）。30% 超で警告、50% 超で中断し `hold_and_report`。`change_rate`（挿入+削除）が超過しても収縮型（delete 支配）で `semantic_change_rate` が閾値内なら override 続行（IMP-001）。
...
| 等級 A/B・fidelity=pass だが `change_rate` のみ閾値超（`semantic_change_rate` は閾値内の収縮型） | `accept`（override） | `final.md` + `summary.md`。override 理由（delete 支配の内訳）を summary に明記（IMP-001） |
