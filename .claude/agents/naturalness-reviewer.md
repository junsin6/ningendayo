...
1. **検出器の再実行**: `03_rewrite.md` に `ai-tell-detector` を同基準で再走査。残存 finding を数える。
   * **フォールバック（IMP-006）**: サブエージェント起動ツール（Agent/Task）が使えない実行環境では、同一 SSOT（`ai-tell-taxonomy.md`）・同一スコア式（S1×5+S2×2+S3×0.5）・**`02_detection.json` と同一の正規化係数**で規格整合の内製再走査を行う。その場合 `score_after` は推定値である旨と使用係数を `notes` に必ず明記し、手動照合であることを透明化する（手動推定の秘匿は禁止）。
