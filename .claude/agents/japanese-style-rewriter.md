...
5. **変更率監視**（IMP-001, 改訂 2026-07-21）: 区間マージ後の**文字レベル編集距離**を基本の change_rate とし、`insert`/`delete` 率を分離併記する（単純加算・sum-metric は使わない）。
   * 30% 超（編集距離ベース）→ `warnings` に記録して続行。
   * 50% は「意味改変 edit 比率」で判定。装飾・常套句・過剰敬語の**純削除**（意味を落とさない削除）は分子から控除し、`del ≫ ins` の削除主導かつ fidelity 保全なら中断しない。ジャンル移動・意味の痩せがあるときのみ `hold_and_report`。
