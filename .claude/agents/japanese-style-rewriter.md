...
2. **finding 順処理**: 各 finding の span を playbook レシピで修正。検出のない区間は一切触らない。`scope` が `scattered`/`document` の finding は代表 `start`/`end` だけでなく **`related_spans` の全出現位置**を対象にする（reason 本文から位置を拾う旧運用は不要）。`merged_categories` を持つ span は主分類のレシピを軸に、従カテゴリも同時に解消する。
3. **連鎖調整**: 同カテゴリの反復（例 A-1「における」5 回）は、全部を同じ形に直さず複数の自然形に分散させる（機械的均一を避ける）。**「意味語＋AIクセ」の混合 span はクセ層だけを剥がす**（例「予定となっております」→『予定』を残し状態叙述のみ除去して「予定です」。意味語まで落とすと fidelity #14 に抵触）。
