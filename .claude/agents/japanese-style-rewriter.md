...
5. **変更率監視**（IMP-001・playbook §変更率の数え方 v1.1 準拠）: `change_rate`（総）・`insertion_rate`（挿入/原文長）・`deletion_rate`（削除/原文長）を分離計上する。
   * `change_rate` 30% 超 → `warnings` に記録して続行（単独では中断しない）。
   * **中断（`hold_and_report`）は意味改変基準で判定**: `insertion_rate` が 30% 超、または意味改変 edit（語義・主張・序列が動きうる置換）が多いとき。純削除・純圧縮が主因（`deletion_rate` ≫ `insertion_rate`）なら `change_rate` 50% 超でも中断せず、内訳を `warnings` に明記して accept 提案する。
   * カタカナ縮約・装飾/常套句の純削除で char-level `change_rate` は容易に膨張する。最終判断は fidelity を優先。
