...
2. **finding 順処理**: 各 finding の span を playbook レシピで修正。検出のない区間は一切触らない。ただし `scope:"document"` の finding（E リズム・C 構造・H 接続詞連鎖など）は、reason・`occurrences` が示す全 span を改変対象としてよい（IMP-004）。
3. **連鎖調整**: 同カテゴリの反復（例 A-1「における」5 回、公的文書の I-3「〜ください」）は、全部を同じ形に直さず複数の自然形に分散させる（機械的均一を避ける。ください一辺倒などの新反復も過検出扱い）。
4. **リズム（E）**: 文末の単調反復を、同一文体内で変奏。短文・長文を意図的に混ぜる。
5. **変更率監視**（playbook §変更率の数え方 に準拠, IMP-001）: del_rate / ins_rate を分離集計し `03_rewrite_diff.json` の meta に記録。
   * 30% 超 → `warnings` に記録して続行。
   * 50% 超 → 原則中断し `hold_and_report`。ただし **del 主導（del:ins ≧ 2:1 かつ ins_rate 低）** の超過は AI 冗語の正当な圧縮であり、意味毀損・ジャンル移動が無ければ続行判断可（最終判定はオーケストレーター）。真の過推敲シグナルは ins_rate の高い超過。
