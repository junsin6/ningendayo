...
5. **変更率監視（IMP-001 適用 v1.1）**: 総 `change_rate`（挿入＋削除／原文）を計算し、必ず**二軸に分離**して `meta` に記録する。
   * `lexical_substitution_rate` = 1:1 語句置換の改変文字数 / 原文文字数。
   * `structural_delete_rate` = 純削除＋純挿入（段落統合・箇条書き溶解・装飾削除）/ 原文文字数。
   * `structural_delete_rate` が 30% 超 → 過推敲の疑いを `warnings` に記録。
   * 総 `change_rate` が 50% 超でも、内訳が語句置換主導なら**中断しない**（`warnings` に「密度由来・要 override 判定」と記し、中断判定はオーケストレーターへ委ねる）。純粋な総変更率だけを理由にした `hold_and_report` は返さない。
