...
{
  "edits": [
    { "finding_id": "f001", "category": "A-6", "type": "lexical", "before": "課題となっている", "after": "課題だ", "rationale": "状態叙述を断定へ" }
  ],
  "change_rate": 0.18,
  "lexical_change_rate": 0.11,
  "structural_deletion_rate": 0.07,
  "style_preserved": "desu_masu",
  "warnings": []
}
```

各 edit に `type: "lexical"`（語句の置換・語形変更）か `"structural"`（装飾・常套句・冗語の純削除、構造編集）を付す。meta の 3 率は playbook §変更率の数え方（v1.1）に従い、`lexical_change_rate + structural_deletion_rate ≈ change_rate` を満たすこと。
...
5. **変更率監視**（IMP-001 分離計上）: `change_rate`（総）・`lexical_change_rate`（語句改変）・`structural_deletion_rate`（純削除）を計算。判定は**実質改変率 = 語句改変率 + 正味挿入増加分**（純削除を除外）で行う。
   * 実質改変率 30% 超 → `warnings` に記録して続行。
   * 実質改変率 50% 超 → 中断し、オーケストレーターへ `hold_and_report` を返す。
   * 総 `change_rate` が閾値超でも実質改変率が閾値未満なら中断しない（削除主導ケース。オーケストレーターが override accept を判断）。
