...
5. **変更率監視（v1.1 — IMP-005）**: change_rate は per-edit 合算ではなく**推敲文全体の difflib グローバル diff** で計測（重複 span の二重計上を避ける）。分母は改行込みの原文全文字数で固定。
   * 30% 超 → `warnings` に記録して続行。
   * 50% 超 → 中断し、オーケストレーターへ `hold_and_report` を返す。
6. **統合 edit の記録**: 同一 span で複数 finding を 1 回で書き換えた場合、diff の当該 edit に `merged_finding_ids: [...]` を付す。統合 edit は単独では起きない意味変化（評価語の脱落等）が紛れやすいため、fidelity-auditor の重点監査対象となる。
