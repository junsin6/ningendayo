...
5. **スコア算出**（正準式は taxonomy §検出出力スキーマ「正規化式」に一元化。検出器はこの式のみ使う）:
   * `W = S1×5 + S2×2 + S3×0.5`（加重和）
   * `severity_weighted_score = round(100 * (1 - exp(-W / 45)), 1)`。係数 k=45 は SSOT の既存例 raw56→71.5 から確定。0 起点・飽和型で高密度短文でも 100 に張り付かない。input_length 非依存（W のみで決定）。
   * 旧式 `100*(1-exp(-W/60))` や `100*W/(W+45)` は使わない（検出器間ブレ IMP-002 の原因）。用いた計算を `meta.notes` に残す。
   * `ai_tell_density` = 検出 span 総文字数 / 全体文字数（規模情報はこちらで表す）。
   * 文書レベル finding（E-1/E-2/C 系）は `scope:"document"` とし、start/end は代表位置アンカーでよい。
