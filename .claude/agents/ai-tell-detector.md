...
5. **スコア算出**（taxonomy「検出出力スキーマ」の定義に厳密準拠 / IMP-002 v1.1）:
   * `raw = S1×5 + S2×2 + S3×0.5` → `raw_density = raw / (input_length/100)` → `severity_weighted_score = round(100×(1−exp(−raw_density/8)), 1)`。**素朴な `min(100, raw)` クリップは禁止**（高密度短文で飽和し解像度を失う）。
   * `ai_tell_density` = 実 AI クセ文字の union 被覆 / 全体文字数。**文書レベル finding の広域代表 span は分子から除外**（重複水増し防止 / IMP-004）。
