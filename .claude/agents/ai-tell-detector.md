...
5. **スコア算出**（IMP-002 で式を固定。便宜式を使わない）:
   * `raw` = S1×5 + S2×2 + S3×0.5（加重和）。
   * `severity_weighted_score = min(100, (raw / input_length × 100) / REF × 100)`、**REF = 15**。
     * `raw / input_length × 100` が 100 字あたり加重和（per100）。`REF=15`（100字あたり15点で満点）で割り ×100 して 0〜100 に正規化。
     * **検算**: 001 raw106.5/L798→per100 13.35→13.35/15×100=**89.0**、002 raw33.5/L685→per100 4.89→**32.6**。この値にならなければ式が誤り。
     * ⚠️ `REF` は per100 基準の 15。生密度 `raw/input_length`（≈0.15）と混同して `REF=0.15` を入れると 100 倍過大で飽和する。高密度短文で即 saturate する旧「raw クリップ」も禁止。REF は実データ蓄積で再較正（`要再較正`）。
   * `ai_tell_density` = 検出 span 総文字数 / 全体文字数。**span 重複時は区間マージ後の非重複結合長**で数える（IMP-005）。`scope:"document"` finding は加算しない。
