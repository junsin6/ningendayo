...
5. **スコア算出**（IMP-002 で正規化式を確定。全 run・全レビュアーで同一式を使う）:
   * `raw` = Σ(S1×5 + S2×2 + S3×0.5)。
   * `severity_weighted_score` = `round(100 * (1 - exp(-raw / 41)), 1)`。
     * 飽和定数 `k = 41`（固定・input_length 非依存）。基準 run `2026-06-12-001`（raw106→92.5）を再現するよう逆算した値。分母は文字数に依存させない指数飽和型で、高密度短文での 100 張り付きを避ける。
     * この式は推敲後の再計測（`05_*`）でも同一に用いること。run 間・detector 間で `k` を変えてはならない。
   * `ai_tell_density` = 検出 span の**重複を除いた union 文字数** / 全体文字数（scattered/document パターンの locator 二重計上を避ける）。
