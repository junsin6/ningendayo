...
5. **スコア算出（IMP-002 適用 v1.1）**:
   * `raw = S1×5 + S2×2 + S3×0.5`。`severity_weighted_score = 100 × raw / (raw + 50)`（**K=50 固定**・半飽和点 raw=50）。K は run 間で固定し、文書長で動かさない。用いた式を `meta.normalization_note` に明記。
   * `ai_tell_density` = 検出 span の**ユニオン被覆**文字数（重複 span は二重計上しない）/ 全体文字数。
