...
5. **スコア算出**（SSOT: `ai-tell-taxonomy.md §検出出力スキーマ` の正規化式に厳密準拠。run 間・agent 間で式を変えない）:
   * `raw` = S1×5 + S2×2 + S3×0.5。
   * `severity_weighted_score` = **100×(1−e^(−raw/40))**（飽和関数。0〜100。参照 raw92→90.0, raw106→92.9）。
   * `ai_tell_density` = 検出 span の**和集合（union）**文字数 / 全体文字数（重複区間を二重計上しない）。
