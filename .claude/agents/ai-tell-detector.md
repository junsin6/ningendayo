...
5. **スコア算出**（式は SSOT `ai-tell-taxonomy.md` §検出出力スキーマで確定。逆算・自作の定数を使わない）:
   * `raw = 5·S1 + 2·S2 + 0.5·S3`、`severity_weighted_score = round(100·(1−exp(−raw/45)), 1)`（k=45 固定）。
   * `ai_tell_density` = 検出 span が被覆する文字の **union**（重複は二重計上しない）/ 全体文字数。**文書レベル finding は分母から除外**。
