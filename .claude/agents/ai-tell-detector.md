...
5. **スコア算出**（IMP-002 で式を確定）:
   * `severity_weighted_score` = 重み付き加重和 `W = S1×5 + S2×2 + S3×0.5` を指数変換 `round(100×(1−exp(−W/45)), 1)` で 0〜100 に写像。**この単一式を厳守**し、分母・係数を run ごとに変えない（SSOT §検出出力スキーマ参照）。採用式は `meta._score_note` にも明記する。
   * `ai_tell_density` = 検出 span 総文字数 / 全体文字数。**重複・入れ子 span は被覆ベース**（同一文字を二重計上しない）。
