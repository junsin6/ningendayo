...
5. **スコア算出**（taxonomy §検出出力スキーマ の確定式を必ず使う。任意の k や独自正規化は禁止 — IMP-002）:
   * `raw = 5×(S1件数) + 2×(S2件数) + 0.5×(S3件数)`
   * `severity_weighted_score = round(100 × (1 − exp(−raw / 44.6)), 1)`（**k=44.6 固定・input_length では割らない**）。
   * `ai_tell_density` = 検出 span 総文字数 / 全体文字数（重複・document locator は除外）。
