...
4. **密度判定**: S2/S3 は**反復回数**を `reason` に明記（例「『における』が 5 回」）。単発を過検出しない。
   * **A-8 / A-8a の分岐（v1.1）**: 受動文は「によって」句で行為者を後置するものを `A-8`（by-passive）、行為者を表に出さない単純受動「〜される／〜された」を `A-8a`（行為者ぼかしの単純受動）として分類する。「変更される／実施される／分別される」型は `A-8a`。誤って A-8 に束ねない。
5. **スコア算出**（taxonomy v1.1 §検出出力スキーマに一意確定）:
   * `raw = 5×S1 + 2×S2 + 0.5×S3`。
   * `severity_weighted_score = 100 × (1 − exp(−raw / 40))`（k=40 固定・飽和正規化）。
   * `ai_tell_density` = 検出 span 総文字数 / 全体文字数（**重複 span は union を取り二重計上しない**）。
   * 出力後に全 finding で `text[start:end] == text_span` を自己検証し、不一致なら修正してから JSON 化。
