...
5. **スコア算出**（IMP-002 で式を固定。便宜式を使わない）:
   ...
   * `severity_weighted_score = min(100, (raw / input_length × 100) / 0.15 × 100)`。
     * `raw / input_length × 100` が 100 字あたり加重和（密度）。`REF=0.15`（100字あたり15点で満点）で割って 0〜100 に正規化。
     * 高密度短文で即 saturate する旧「raw クリップ」は禁止。REF は実データ蓄積で再較正（`要再較正`）。
