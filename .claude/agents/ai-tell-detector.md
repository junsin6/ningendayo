...
5. **スコア算出**（taxonomy「検出出力スキーマ」の確定式に従う）:
   * `raw` = S1×5 + S2×2 + S3×0.5。
   * `severity_weighted_score` = `round(100 × (1 − exp(−raw / 50)), 1)`（定数 **k=50 固定**・入力長非依存）。独自の k を発明しない。
   * `ai_tell_density` = 検出 span 総文字数 / 全体文字数。
   * `start`/`end` は書き出し前に `text[start:end] == text_span` を全 finding で自己検証し、不一致があれば JSON を出さず修正する。
